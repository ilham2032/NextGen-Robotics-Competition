...
import { signInReferee } from '../../referee/auth'

const TeamsZoneRefereeLogin = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setTimeout(() => navigate('/teamszone/referee/dashboard'), 800)
    } catch {
    ...
    <section className="min-h-screen bg-gradient-to-br from-blue-900 via-blue-700 to-cyan-500 px-6 py-24">
      <div className="mx-auto max-w-md rounded-3xl border border-white/30 bg-white p-8 shadow-2xl">
        ...
        <p className="mt-6 text-xs text-slate-500 text-center">
          Referee accounts are created by administrators. Contact the event organizer if you need access.
        </p>
      </div>
    </section>
  )
...
export default TeamsZoneRefereeLogin
