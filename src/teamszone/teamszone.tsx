    ...
    <section className="min-h-screen bg-linear-to-br from-slate-50 via-blue-50 to-cyan-50 px-4 pb-16 pt-24 sm:px-6 sm:pt-28">
      <div className="mx-auto max-w-6xl">
        <div className="rounded-3xl border border-blue-100 bg-linear-to-r from-blue-800 via-indigo-800 to-blue-900 px-6 py-10 text-center text-white shadow-lg sm:px-10">
          <p className="text-xs font-semibold tracking-[0.28em] text-blue-200 uppercase">{t("Live Competition Hub")}</p>
          <h1 className="mt-3 font-display text-4xl font-bold sm:text-5xl lg:text-6xl">{t("Teams Zone")}</h1>
        ...
        <div className="mt-10 grid grid-cols-2 gap-4 md:grid-cols-4">
          <div className="rounded-2xl border border-blue-100 bg-white p-6 text-center shadow-sm">
            ...
            <div className="text-sm font-medium text-slate-600">{t("Total Teams")}</div>
          </div>
          <div className="rounded-2xl border border-blue-100 bg-white p-6 text-center shadow-sm">
            <div className="text-3xl font-bold text-blue-600 mb-1">
              {teams.reduce((sum, team) => sum + (team.members || 0), 0)}
            </div>
            <div className="text-sm font-medium text-slate-600">{t("Participants")}</div>
          </div>
          <div className="rounded-2xl border border-blue-100 bg-white p-6 text-center shadow-sm">
            <div className="text-3xl font-bold text-blue-600 mb-1">{filteredCategories.length}</div>
            <div className="text-sm font-medium text-slate-600">{t("Categories")}</div>
          </div>
          <div className="rounded-2xl border border-blue-100 bg-white p-6 text-center shadow-sm">
            <div className="text-3xl font-bold text-blue-600 mb-1">{totalBattles}</div>
            <div className="text-sm font-medium text-slate-600">{t("Battles Played")}</div>
          </div>
        </div>

        <div className="mt-10">
          <h2 className="text-2xl font-semibold text-slate-800 mb-6 text-center">{t("Competition Categories")}</h2>
