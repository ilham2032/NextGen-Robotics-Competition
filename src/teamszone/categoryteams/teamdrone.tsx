...
const TeamDrone = () => {
  ...
  const category = categories.find(cat => cat.name === 'Drone')
  const categoryTeams = allTeams.filter(team => team.categoryName?.trim() === 'Drone')
    ...
    <section className="min-h-screen bg-gradient-to-br from-slate-50 to-gray-50 px-4 pb-16 pt-24 sm:px-6 sm:pt-28">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="text-center mb-8">
          <Link
            to="/teamszone"
            className="inline-flex items-center text-slate-600 hover:text-slate-700 mb-4 transition-colors"
          >
          ...
          <h1 className="font-display text-3xl font-bold text-slate-700 sm:text-4xl lg:text-5xl mb-2">
            Drone Teams
          </h1>
          <p className="text-lg text-slate-600 max-w-2xl mx-auto">
            {category?.description || 'Aerial robotics and drone competition teams'}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4 text-center">
            <div className="text-2xl font-bold text-slate-600 mb-1">{categoryTeams.length}</div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4 text-center">
            <div className="text-2xl font-bold text-slate-600 mb-1">{uniqueCountries.size}</div>
            <div className="text-sm text-slate-600 font-medium">Countries</div>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4 text-center">
            <div className="text-2xl font-bold text-slate-600 mb-1">Aerial</div>
        ...
        <div className="mb-8">
          {categoryTeams.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-xl shadow-sm border border-slate-100">
              ...
              <p className="text-slate-600 mb-6">Be the first to register a team in the Drone category!</p>
              <Link
                to="/contact"
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {categoryTeams.map((team) => (
                <article key={team.id} className="bg-white rounded-xl shadow-sm border border-slate-100 p-6 hover:shadow-md transition-shadow">
        ...
        {category && (
          <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
            <h3 className="text-lg font-semibold text-slate-800 mb-3">About Drone Competition</h3>
            ...
            <div className="flex flex-wrap gap-4 text-sm">
              <div className="flex items-center text-slate-600">
                ...
                <Link to="/regulations" className="text-slate-600 hover:text-slate-700">View Regulations</Link>
              </div>
              <div className="flex items-center text-slate-600">
                <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
                <Link to={`/rules/drone`} className="text-slate-600 hover:text-slate-700">Competition Rules</Link>
              </div>
            </div>
          </div>
        )}
