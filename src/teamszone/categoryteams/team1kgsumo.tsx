...
const Team1kgSumo = () => {
  ...
  const category = categories.find(cat => cat.name === '1kg Lego Sumo')
  const categoryTeams = allTeams.filter(team => team.categoryName?.trim() === '1kg Lego Sumo')

  const uniqueCountries = new Set(categoryTeams.map(t => t.school).filter(Boolean))

  return (
    <section className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-50 px-4 pb-16 pt-24 sm:px-6 sm:pt-28">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="font-display text-3xl font-bold text-amber-700 sm:text-4xl lg:text-5xl mb-2">
            1kg Lego Sumo Teams
          </h1>
          <p className="text-lg text-slate-600 max-w-2xl mx-auto">
            {category?.description || 'LEGO-based sumo competition with 1kg robot limit'}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-white rounded-xl shadow-sm border border-amber-100 p-4 text-center">
            <div className="text-2xl font-bold text-amber-600 mb-1">{categoryTeams.length}</div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-amber-100 p-4 text-center">
            <div className="text-2xl font-bold text-amber-600 mb-1">{uniqueCountries.size}</div>
            <div className="text-sm text-slate-600 font-medium">Countries</div>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-amber-100 p-4 text-center">
            <div className="text-2xl font-bold text-amber-600 mb-1">1kg</div>
        ...
        <div className="mb-8">
          {categoryTeams.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-xl shadow-sm border border-amber-100">
              ...
              <p className="text-slate-600 mb-6">Be the first to register a team in the 1kg Lego Sumo category!</p>
              <Link
                to="/contact"
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {categoryTeams.map((team) => (
                <article key={team.id} className="bg-white rounded-xl shadow-sm border border-amber-100 p-6 hover:shadow-md transition-shadow">
                  ...
                  <div className="pt-4 border-t border-slate-100">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-slate-500">1kg Lego Sumo</span>
                    </div>
                  </div>
                </article>
              ))}
        ...
        {category && (
          <div className="bg-white rounded-xl shadow-sm border border-amber-100 p-6">
            <h3 className="text-lg font-semibold text-slate-800 mb-3">About 1kg Lego Sumo Competition</h3>
            ...
            <div className="flex flex-wrap gap-4 text-sm">
              <div className="flex items-center text-slate-600">
                ...
                <Link to="/regulations" className="text-amber-600 hover:text-amber-700">View Regulations</Link>
              </div>
              <div className="flex items-center text-slate-600">
                <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
                <Link to={`/rules/1kglegosumo`} className="text-amber-600 hover:text-amber-700">Competition Rules</Link>
              </div>
            </div>
          </div>
        )}
